<script>
  ...
  async function updateStats() {
    ...
    if (!durationRes.error && barChart) {
      ...
      barChart.data.datasets[0].data = durationRes.data.map(
        (stat) => stat.hours
      );
      barChart.update();
    }
    ...
    if (!freqRes.error && frequencyChart) {
      ...
      frequencyChart.data.datasets[0].data = freqRes.data.map(
        (stat) => stat.count
      );
  ...
  onMount(async () => {
    ...
    try {
      const [durationRes, muscleRes, freqRes, heatmapRes, weekdayRes] =
        await Promise.all([
          fetchGet(`/api/stats/weekly-duration/${$user.id}?range=${timeRange}`),
          fetchGet(
            `/api/stats/muscle-distribution/${$user.id}?range=${timeRange}`
          ),
          fetchGet(
            `/api/stats/workout-frequency/${$user.id}?range=${timeRange}`
          ),
          fetchGet(`/api/stats/heatmap/${$user.id}`),
          fetchGet(
            `/api/stats/weekday-frequency/${$user.id}?range=${timeRange}`
          ),
        ]);
          ...
          <div class="heatmap-container">
            <div class="heatmap-grid">
              {#each getCalendarData() as day}
                <div
                  class="day-cell level-{Math.min(day.count, 4)}"
...
<style>
  ...
  :global(body.dark-mode) .range-controls {
    background: #374151;
  }
  :global(body.dark-mode) .range-btn {
    color: #9ca3af;
  }
  :global(body.dark-mode) .range-btn.active {
    background: #4b5563;
    color: #fff;
  }

  :global(body.dark-mode) .day-cell {
    background-color: #2d333b;
  }
  :global(body.dark-mode) .day-cell.level-1 {
    background-color: #0e4429;
  }
  :global(body.dark-mode) .day-cell.level-2 {
    background-color: #006d32;
  }
  :global(body.dark-mode) .day-cell.level-3 {
    background-color: #26a641;
  }
  :global(body.dark-mode) .day-cell.level-4 {
